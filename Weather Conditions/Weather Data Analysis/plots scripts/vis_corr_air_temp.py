...
def airTemp_correlation(csv_file_path):
    """
    Read a CSV file containing cleaned F1 merged data into a Pandas DataFrame.
    Calculate the correlation coefficient between Air Temperature and Points earned by each driver.
    ...
    for driver in drivers:
        ...
        correlation_coefficient, _ = pearsonr(driver_data['AirTemp'], driver_data['points'])

        correlation_coefficients.append(correlation_coefficient)

    # Plot correlation coefficients as a bar graph
    plt.bar(drivers, correlation_coefficients, color='black')  # Set color to black
    plt.xlabel('Driver')
    plt.ylabel('Correlation Coefficient')
    celsius_symbol = u'\u00b0C'
    plt.title(f'Corr. Coeff. Between Air Temperature ({celsius_symbol}) and Points Earned by Each Driver')
    plt.show()
...
airTemp_correlation(csv_file_path)
