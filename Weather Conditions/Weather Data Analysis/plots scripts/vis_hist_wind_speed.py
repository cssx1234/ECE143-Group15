...
def windspeed_histogram(csv_file_path):
    """
    Read a CSV file containing cleaned F1 merged data into a Pandas DataFrame.
    Calculate the correlation coefficient between Wind Speed and Points earned by each driver.
    ...
    for driver in drivers:
        ...
        correlation_coefficient, _ = pearsonr(driver_data['WindSpeed'], driver_data['points'])
    ...
    plt.title('Histogram of Corr. Coeff. Between Wind Speed (knots) and Points Earned by Drivers')
    plt.show()

csv_file_path = 'cleaned_f1_merged_data.csv'

# Call the function with the cleaned CSV file path
windspeed_histogram(csv_file_path)
